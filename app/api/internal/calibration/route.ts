...
function unauthorized(message: string, status = 403): NextResponse {
...
function buildRegimeDistributionSimple(input: {
  STABLE: DecisionDistribution;
  TRANSITION: DecisionDistribution;
  VOLATILE: DecisionDistribution;
}) {
  return {
    STABLE: buildDistributionSimple(input.STABLE),
    TRANSITION: buildDistributionSimple(input.TRANSITION),
    VOLATILE: buildDistributionSimple(input.VOLATILE),
  };
}
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const calibrationResult = buildCalibrationOrchestratorResult({
      samples,
      analytical_version: analyticalVersion,
      horizon,
      persist_state: false,
    ...
    return NextResponse.json(
      {
        ...
        data: {
          ...
          decision_calibration_state: calibrationResult.state,
        ...
        meta: {
          ...
          effective_sample_size: calibrationResult.effective_sample_size,
        },
      },
      {
        status: 200,
